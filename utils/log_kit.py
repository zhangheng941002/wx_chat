...
def getLogger(file_name="task"):
    fh = MyLoggerHandler(file_name + ".log", encoding='utf-8')
    logger = logging.getLogger(file_name)
    logger.setLevel(logging.DEBUG)
    fm = logging.Formatter(
        "%(asctime)s %(filename)s func:%(funcName)s line %(lineno)d  %(levelname)s message: %(message)s")
